...
public class CorporateTransactionServiceImpl implements CorporateTransactionService {
    ...
    public void checkStatus(CorporateTransaction corporateTransaction) {
        List<TransactionSigner> signerList = transactionSignerService.findByCorporateTransaction(corporateTransaction);
        long rejectCount = signerList.stream().filter(signer -> signer.getOperationType().equals(OperationType.REJECT)).count();
        if (rejectCount > 0) {
            //todo transaction status should be REJECT in here

            corporateTransaction.setStatus(TransactionStatus.DONE);
            ...
            //todo transaction status should be READY in here
